...
export default function Share({ share }){
    ...
    const submit = (e) => {
        e.preventDefault();
        patch(route('share.update', share.id), { onSuccess: () => setEditing(false) });
    };
        ...
        <div className="flex-1">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <small className="ml-2 text-sm text-gray-600">{dayjs(share.created_at).fromNow()}</small>
                    { share.created_at !== share.updated && <small className="text-sm text-gray-600"> &middot; edited</small>}
                </div>
                ...
                { editing
                    ...
                    :  <p className="mt-4 text-lg text-gray-900">{share.message}</p>
                }
